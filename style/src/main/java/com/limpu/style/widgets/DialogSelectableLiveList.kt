...
import com.limpu.style.databinding.DialogBottomSelectableListItemBinding
import com.limpu.style.databinding.DialogSelectableListBinding
import java.util.*
 ...
 * 圆角的文本框底部弹窗
 */
class DialogSelectableLiveList<T> :
     TransparentDialog<DialogSelectableListBinding>() {
    ...
    fun setTitle(@StringRes title: Int): DialogSelectableLiveList<T> {
        initTitle = title
        return this
    }

    fun setText(text: String?): DialogSelectableLiveList<T> {
        initText = text
        return this
    }

    fun setDataLoader(loader: DataLoader<T>): DialogSelectableLiveList<T> {
        dataLoader = loader
        return this
    }

    fun setHint(@StringRes hint: Int): DialogSelectableLiveList<T> {
        initHint = hint
        return this
    }

    fun setInitValue(value: T?): DialogSelectableLiveList<T> {
        initSelected = value
        return this
    }

    fun setOnConfirmListener(onConfirmListener: OnConfirmListener<T>): DialogSelectableLiveList<T> {
