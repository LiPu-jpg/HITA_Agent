...
class DocxFileParser : FileParser {
    ...
    private fun extractTextFromDocx(file: File): String {
        ...
        LogUtils.d( "开始解压DOCX文件: ${file.absolutePath}, 文件大小: ${file.length()}")
        ...
        ZipInputStream(file.inputStream()).use { zip ->
            ...
            while (entry != null) {
                entryCount++
                LogUtils.d( "ZIP条目 $entryCount: ${entry.name}, 大小: ${entry.size}")

                if (entry.name == DOCUMENT_XML_PATH) {
                    LogUtils.d( "找到 word/document.xml，开始提取文本")

                    // 读取 XML 内容
                    val xmlContent = zip.reader().readText()
                    LogUtils.d( "XML内容长度: ${xmlContent.length}, 预览: ${xmlContent.take(200)}")

                    // 提取 <w:t> 标签内的文本
                    val textPattern = Regex("<w:t[^>]*>(.*?)</w:t>", RegexOption.DOT_MATCHES_ALL)
                    val matches = textPattern.findAll(xmlContent).toList()

                    LogUtils.d( "找到 ${matches.size} 个 <w:t> 标签")
            ...
            if (entryCount == 0) {
                LogUtils.e( "ZIP文件中没有找到任何条目！")
            } else {
                LogUtils.e( "ZIP文件中没有找到 $DOCUMENT_XML_PATH")
            }
        }
