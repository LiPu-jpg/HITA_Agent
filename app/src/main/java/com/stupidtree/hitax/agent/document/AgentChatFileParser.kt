...
object AgentChatFileParser {
    ...
    suspend fun processAttachment(
        ...
        return withContext(Dispatchers.IO) {
            android.util.Log.d(TAG, "========== 开始处理附件 ==========")
            android.util.Log.d(TAG, "📝 用户输入: ${userText.take(50)}...")
            ...
            try {
                ...
                when (parseResult) {
                    is ParseResult.Success -> {
                        android.util.Log.d(TAG, "✅ 本地解析成功")
                        android.util.Log.d(TAG, "📝 文本长度: ${parseResult.text.length}")
                    ...
                    is ParseResult.NeedCloud -> {
                        android.util.Log.d(TAG, "📤 需要云端处理: ${parseResult.reason}")
                        ...
                        android.util.Log.d(TAG, "📊 文件大小: ${fileBytes.size} bytes")

                        val base64Content = Base64.encodeToString(fileBytes, Base64.NO_WRAP)
                        android.util.Log.d(TAG, "✅ Base64编码完成: ${base64Content.length} 字符")
                    ...
                    is ParseResult.Error -> {
                        android.util.Log.e(TAG, "❌ 解析失败: ${parseResult.message}")
                ...
                android.util.Log.e(TAG, "❌ 附件处理异常", e)
                FileProcessResult.Error(
