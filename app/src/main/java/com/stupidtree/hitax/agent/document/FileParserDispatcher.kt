    ...
    suspend fun parse(context: Context, uri: Uri, fileName: String): ParseResult {
        return withContext(Dispatchers.IO) {
            try {
                ...
                Log.d(TAG, "文件大小: ${formatFileSize(fileSize)}")
                ...
                Log.d(TAG, "使用解析器: ${parser::class.simpleName}")

                // 6. 执行解析
                val result = parser.parse(context, uri, fileName)

                Log.d(TAG, "解析结果: ${result::class.simpleName}")
                Log.d(TAG, "========== 文件解析完成 ==========")
