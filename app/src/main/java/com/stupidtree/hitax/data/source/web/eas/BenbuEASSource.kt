...
class BenbuEASSource : EASService {
        ...
        executor.execute {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    Log.d(TAG, "getTimetableOfTerm term=${term.getCode()} rawCourseCount=${parsedCourses.size} mergedCourseCount=${courses.size} cookieKeys=${token.cookies.keys.sorted()} ${cookieFingerprintSummary(token.cookies)}")
    ...
    private fun requestBenbuScores(
        ...
        Log.d(TAG, "requestBenbuScores term=${term.getCode()} name=${term.termName} testType=$testType path=$path params=$params")
