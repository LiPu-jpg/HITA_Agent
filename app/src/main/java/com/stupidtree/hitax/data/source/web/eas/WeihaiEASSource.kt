...
class WeihaiEASSource : EASService {
    ...
    companion object {
        private const val TAG = "WeihaiEASSource"
        ...
        executor.execute {
            try {
                ...
                if (scoreFetch.authExpired && timetableFetch.authExpired) {
                    Log.w(TAG, "getAllTerms auth expired on both term pages")
                ...
                Log.d(TAG, "getAllTerms score=${scoreTerms.map { it.getCode() }} timetable=${timetableTerms.map { it.getCode() }}")
                Log.d(TAG, "getAllTerms parsed=${visibleTerms.map { "${it.getCode()}:${it.name}:current=${it.isCurrent}" }}")
    ...
    override fun getStartDate(token: EASToken, term: TermItem): LiveData<DataState<Calendar>> {
        ...
        executor.execute {
            try {
                ...
                Log.d(TAG, "getStartDate term=${term.getCode()} inferredStart=${start.time} termName=${term.termName} label=${term.name}")
    ...
    override fun getTimetableOfTerm(
        ...
        executor.execute {
            try {
                ...
                Log.d(TAG, "getTimetableOfTerm request path=/kbcx/queryGrkb params={fhlj=kbcx/queryGrkb,xnxq=$termCode}")
                ...
                if (isWeihaiAuthExpiredResponse(response, body)) {
                    Log.w(TAG, "getTimetableOfTerm auth expired term=$termCode status=$statusCode")
                    result.postValue(DataState(DataState.STATE.NOT_LOGGED_IN))
                    return@execute
                }
                ...
                Log.d(TAG, "getTimetableOfTerm parsed term=$termCode rawCourseCount=${parsedCourses.size} mergedCourseCount=${courses.size}")
    ...
    override fun getScheduleStructure(
        ...
        executor.execute {
            try {
                ...
                Log.d(TAG, "getScheduleStructure term=${term.getCode()} courseCount=${courses.size} maxPeriod=$maxPeriod resolvedSize=${resolved.size}")
                result.postValue(DataState(resolved, DataState.STATE.SUCCESS))
            } catch (e: Exception) {
                Log.w(TAG, "getScheduleStructure fallback to default: ${e.message}")
    ...
    private fun fetchTermDoc(token: EASToken, url: String, selectName: String): TermDocFetchResult {
        ...
        fun parseValidTermDoc(response: Connection.Response): Document? {
            ...
            Log.w(TAG, "fetchTermDoc missing selector=$selectName url=$url title=$title sample=$sample")
            return null
        }
        ...
        return try {
            ...
            if (doc == null) {
                Log.d(TAG, "fetchTermDoc retry by missing selector select=$selectName url=$url status=${response.statusCode()}")
                authExpired = true
            }

            if (authExpired) {
                Log.w(TAG, "fetchTermDoc auth expired url=$url select=$selectName status=${response.statusCode()}")
            ...
            Log.w(TAG, "fetchTermDoc failed url=$url select=$selectName err=${e.message}")
            TermDocFetchResult(null, false)
        }
    }
    ...
    private fun getTimetableOfTermSync(term: TermItem, token: EASToken): List<CourseItem> {
        ...
        val courses = mergeAdjacentCourses(parsedCourses)
        logEmptyTimetableDetails(term, body, courses)
        Log.d(TAG, "getTimetableOfTermSync term=${term.getCode()} rawCourseCount=${parsedCourses.size} mergedCourseCount=${courses.size} cookieKeys=${token.cookies.keys.sorted()} ${cookieFingerprintSummary(token.cookies)}")
        return courses
    }
    ...
    private fun requestWeihaiScores(
        ...
        Log.d(TAG, "requestWeihaiScores term=${term.getCode()} name=${term.termName} testType=$testType path=$path params=$params")
