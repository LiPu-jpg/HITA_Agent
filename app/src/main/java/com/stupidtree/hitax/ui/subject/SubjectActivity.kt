...
import com.stupidtree.hitax.agent.subject.SubjectReadmeAgentFactory
import com.stupidtree.hitax.agent.subject.SubjectReadmeAgentInput
import com.stupidtree.hitax.agent.subject.SubjectReadmeAgentOutput
...
class SubjectActivity : BaseActivity<SubjectViewModel, ActivitySubjectBinding>(),
    ...
    companion object {
        private const val TAG = "SubjectReadmeDebug"
    }
    ...
    private fun loadReadmeForSubject(subject: TermSubject) {
        ...
        session.run(
            ...
            onTrace = { trace ->
                Log.d(TAG, "agent trace: stage=${trace.stage} message=${trace.message} payload=${trace.payload}")
                runOnUiThread {
            ...
            onResult = { result ->
                runOnUiThread {
                    if (!result.ok) {
                        Log.d(TAG, "agent resolve failed: ${result.error}, fallback to direct resolver")
        ...
        when (candidates.size) {
            ...
            1 -> {
                ...
                if (repoName.isBlank()) {
                    ...
                    Log.d(TAG, "loadReadmeForSubject: auto-selected repo=$repoName")
        ...
        readmeObserver = Observer { state ->
            when (state.state) {
                ...
                DataState.STATE.SUCCESS -> {
                    ...
                    val data = state.data
                    if (data == null) {
                        Log.d(TAG, "observeReadme: success but data null")
                    ...
                    Log.d(TAG, "observeReadme: scopedMarkdownLength=${scoped.length}")
                    ...
                    Log.d(TAG, "observeReadme: failed state=${state.state} msg=$rawMessage")
    ...
    private fun filterReadmeForCandidate(
        ...
        if (markdown.isBlank()) {
            Log.d(TAG, "filterReadmeForCandidate: markdown blank")
            return markdown
        }
        ...
        Log.d(TAG, "filterReadmeForCandidate keys: codeTokens=${keys.codeTokens} nameKeys=${keys.nameKeys}")
        logMarkdownNeighborhood(markdown, "软体机器人理论与技术")

        if (keys.codeTokens.isEmpty() && keys.nameKeys.isEmpty()) {
            Log.d(TAG, "filterReadmeForCandidate: keys empty, return raw markdown")
            return markdown
        }

        extractByTomlCourseMeta(markdown, keys)?.let {
            Log.d(TAG, "filterReadmeForCandidate: matched by TOML meta")
            return it
        }

        val sections = splitReadmeSections(markdown)
        Log.d(TAG, "filterReadmeForCandidate: section count=${sections.size}")
        logSectionHeadings(sections, keys)
        if (sections.size <= 1) {
            Log.d(TAG, "filterReadmeForCandidate: no structured sections, return raw markdown")
            return markdown
        }
        ...
        if (matched == null) {
            Log.d(TAG, "filterReadmeForCandidate: no heading matched, return raw markdown")
            return markdown
        }

        Log.d(TAG, "filterReadmeForCandidate: matched heading='${matched.heading}' level=${matched.level}")
        return buildString {
    ...
    private fun logMarkdownNeighborhood(markdown: String, keyword: String) {
        ...
        if (index < 0) {
            Log.d(TAG, "markdown neighborhood: keyword '$keyword' not found")
            return
        }
        ...
        val snippet = markdown.substring(start, end)
            .replace("\n", "\\n")
            .replace("\r", "")
        Log.d(TAG, "markdown neighborhood around '$keyword': $snippet")
    }
    ...
    private fun logSectionHeadings(sections: List<ReadmeSection>, keys: CourseMatchKeys) {
        sections.forEachIndexed { index, section ->
            ...
            Log.d(TAG, "section[$index] L${section.level} heading='${section.heading}' matched=$hit")
        }
    }
    ...
    private fun extractByTomlCourseMeta(markdown: String, keys: CourseMatchKeys): String? {
        ...
        if (headings.isEmpty()) {
            Log.d(TAG, "extractByTomlCourseMeta: no headings")
            return null
        }
        ...
        for ((index, heading) in headings.withIndex()) {
            ...
            if (attrs == null) {
                Log.d(TAG, "extractByTomlCourseMeta: heading='$title' has no TOML-COURSE")
                continue
            }
            ...
            Log.d(TAG, "extractByTomlCourseMeta: selected heading='$title' level=$level")
            return buildString {
        ...
        Log.d(TAG, "extractByTomlCourseMeta: no TOML section matched")
        return null
    }
